...
HTML;
	...
	if(isset($_POST['userSubmit']) && isset($_POST['userTitle'])){
		...
		$msDate = date("Y-m-d H-i-s");
		$sql = "INSERT INTO `messages`( `uName`, `msTitle`, `msContent`,`mcDate`) VALUES ('".$userName."','".$title."','".$cont."','".$msDate."')";
			...
			if($results = mysqli_query($link,$sql)){
				if(mysqli_num_rows($results) > 0){
						...
						<div>
							...
							<div class='msSpan2'>
								<a href='6.2.showMs.php?msId={$result['msId']}' target='_blank'>
									{$result['msTitle']}
								</a>
							</div>
